...
describe("Matchers", () => {
  ...
  describe("arrayIncludes", () => {
    test("returns true when array contains value", () => {
      expect(
        Matchers_1.arrayIncludes("val").asymmetricMatch(["val", "val2"])
      ).toBe(true);
    });
    test("returns false when array does not contain value", () => {
      expect(
        Matchers_1.arrayIncludes("val3").asymmetricMatch(["val", "val2"])
      ).toBe(false);
    });
    ...
    test("returns false when for null", () => {
      // @ts-ignore
      expect(Matchers_1.arrayIncludes("val3").asymmetricMatch(null)).toBe(
        false
      );
    });
    test("returns false when for undefined", () => {
      // @ts-ignore
      expect(Matchers_1.arrayIncludes("val3").asymmetricMatch(undefined)).toBe(
        false
      );
    });
  });
  describe("mapHas", () => {
    test("returns true when map contains key", () => {
      expect(
        Matchers_1.mapHas("key").asymmetricMatch(new Map([["key", "val"]]))
      ).toBe(true);
    });
    test("returns false when map does not contain key", () => {
      expect(
        Matchers_1.mapHas("key3").asymmetricMatch(new Map([["key", "val"]]))
      ).toBe(false);
    });
    test("returns false when not a map", () => {
      // @ts-ignore
  ...
  describe("setHas", () => {
    test("returns true when set contains value", () => {
      expect(Matchers_1.setHas("val").asymmetricMatch(new Set(["val"]))).toBe(
        true
      );
    });
    test("returns false when set does not contain value", () => {
      expect(
        Matchers_1.setHas("val3").asymmetricMatch(new Set(["val", "val2"]))
      ).toBe(false);
    });
  ...
  describe("objectContainsKey", () => {
    test("returns true when object contains key", () => {
      expect(
        Matchers_1.objectContainsKey("key").asymmetricMatch({ key: "val" })
      ).toBe(true);
    });
    test("returns false when object does not contain key", () => {
      expect(
        Matchers_1.objectContainsKey("key3").asymmetricMatch({ key: "val" })
      ).toBe(false);
    });
    test("returns false when not a object", () => {
      expect(Matchers_1.objectContainsKey("val3").asymmetricMatch(213)).toBe(
        false
      );
    });
    test("returns false when for null", () => {
      expect(Matchers_1.objectContainsKey("val3").asymmetricMatch(null)).toBe(
        false
      );
    });
    test("returns false when for undefined", () => {
      expect(
        Matchers_1.objectContainsKey("val3").asymmetricMatch(undefined)
      ).toBe(false);
    });
  });
  describe("objectContainsValue", () => {
    test("returns true when object contains value", () => {
      expect(
        Matchers_1.objectContainsValue("val").asymmetricMatch({ key: "val" })
      ).toBe(true);
    });
    test("returns false when object does not contain value", () => {
      expect(
        Matchers_1.objectContainsValue("val3").asymmetricMatch({ key: "val" })
      ).toBe(false);
    });
    test("returns false when not a object", () => {
      expect(Matchers_1.objectContainsValue("val3").asymmetricMatch(213)).toBe(
        false
      );
    });
    test("returns false when for null", () => {
      expect(Matchers_1.objectContainsValue("val3").asymmetricMatch(null)).toBe(
        false
      );
    });
    test("returns false when for undefined", () => {
      expect(
        Matchers_1.objectContainsValue("val3").asymmetricMatch(undefined)
      ).toBe(false);
    });
  });
  describe("notNull", () => {
  ...
  describe("notUndefined", () => {
    test("returns true when object", () => {
      expect(Matchers_1.notUndefined().asymmetricMatch({ key: "val" })).toBe(
        true
      );
    });
